...
module.exports = {
  ...
  index: (req, res, next) => {
    Subscriber.find({}) // Find all subscribers (no filter)
      .then(subscribers => {
        res.locals.subscribers = subscribers; // Store in res.locals for access in the view
        next(); // Call the next middleware (usually the view renderer)
      })
      .catch(error => {
        console.log(`Error fetching subscribers: ${error.message}`); // Log errors
  ...
  // Controller to save a new subscriber from form submission
  saveSubscriber: (req, res) => {
    ...
    newSubscriber
      .save()
      .then(result => {
        res.render("thanks"); // Render "thanks" page on success
      })
      .catch(error => {
